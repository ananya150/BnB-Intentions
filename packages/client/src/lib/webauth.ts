...
export class PassKeyKeyPair {
  ...
  static async getValidPassKeyPair(
    webAuthnClient: IWebAuthnClient,
    passkeyIds?: string[],
  ): Promise<PassKeyKeyPair> {
    const authData = await webAuthnClient.authenticate(
    ...
    return new PassKeyKeyPair(
      ...
      BigNumber.from(0),
      webAuthnClient,
      parsedAuth.authenticator.name,
      parsedAuth.authenticator.aaguid,
      parsedAuth.authenticator.name,
      Date.now(),
    );
  }
  ...
  async signChallenge(payload: string): Promise<PassKeySignature> {
    ...
    const authData = await this.webAuthnClient.authenticate(
      challenge,
      [this.keyId],
      { userVerification: "required", authenticatorType: "both" },
    );
